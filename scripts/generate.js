 ...
 * @prop {Record<'base'|'ansi'|'brightOther'|'other', string[]>} bumbleCoffee - Bumble Coffee color variables.
...
const transformLight = theme => {
    ...
    // Map base colors
    const darkBase = theme.bumbleCoffee.base;
    const lightBase = theme.bumbleCoffee.light.base;
    for (let i = 0; i < darkBase.length && i < lightBase.length; i++) {
        colorMap.set(darkBase[i], lightBase[i]);
    }

    // Map ANSI colors
    const darkAnsi = theme.bumbleCoffee.ansi;
    const lightAnsi = theme.bumbleCoffee.light.ansi;
    for (let i = 0; i < darkAnsi.length && i < lightAnsi.length; i++) {
        colorMap.set(darkAnsi[i], lightAnsi[i]);
    }

    // Map brightOther colors
    const darkBrightOther = theme.bumbleCoffee.brightOther;
    const lightBrightOther = theme.bumbleCoffee.light.brightOther;
    for (let i = 0; i < darkBrightOther.length && i < lightBrightOther.length; i++) {
        colorMap.set(darkBrightOther[i], lightBrightOther[i]);
    }

    // Map other colors
    const darkOther = theme.bumbleCoffee.other;
    const lightOther = theme.bumbleCoffee.light.other;
    ...
    light.bumbleCoffee.base = lightBase;
    light.bumbleCoffee.ansi = lightAnsi;
    light.bumbleCoffee.brightOther = lightBrightOther;
    light.bumbleCoffee.other = lightOther;
...
module.exports = async () => {
    const yamlFile = await readFile(
        join(__dirname, '..', 'src', 'bumble-coffee.yml'),
        'utf-8'
    );
